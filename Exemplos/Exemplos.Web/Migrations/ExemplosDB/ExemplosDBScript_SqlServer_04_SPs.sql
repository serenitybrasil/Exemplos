IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME = 'SQLTableOrViewToJSON')
BEGIN
	DROP PROC SQLTableOrViewToJSON
END
GO


CREATE PROCEDURE SQLTableOrViewToJSON

@TableName VARCHAR(MAX) = '',
@ExportPath VARCHAR(MAX) = '',
@ExportName VARCHAR(MAX) = '',
@TableasIdentifier bit = 0
...
IF (@TableasIdentifier IS NULL)
SET @TableasIdentifier = 0
...
--Metadata
	...
	--Build the executable pieces in a table, easier to debug that way
		...
		IF (@TableasIdentifier =1)
		BEGIN
			INSERT INTO SQLStatementsForJSONOutput (Descriptor, StatementString)
			VALUES ('Insert Opening Bracket', 'INSERT INTO StringsForOutput VALUES (''"' + @TableName + '":'')' + @CRLF)
		END
...
--Check for the table to export
	...
	ELSE
		BEGIN
			...
			SELECT String FROM StringsForOutput ORDER BY ID
...
--DROP CREATED TABLES
DROP TABLE [dbo].[SQLStatementsForJSONOutput];

DROP TABLE [dbo].[StringsForOutput];
